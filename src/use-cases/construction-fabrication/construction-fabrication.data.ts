import type { ConstructionDepartment, ConstructionModel, ConstructionTask, LookAheadFilters, LookAheadPeriod, ProjectEntity } from './construction-fabrication.types';
import tasksCsv from './tasks.csv?raw';
import resourcesCsv from './resources.csv?raw';
import lookAheadCsv from './lookahead.csv?raw';
import dependenciesCsv from './dependencies.csv?raw';
...
/** Small RFC4180-style adapter; surplus cells are deliberately preserved in the final notes field. */
export function parseCsv(input: string): CsvRow[] {
  ...
  return lines.map((line) => {
    ...
    values.push(value);
    if (values.length > headers.length) values.splice(headers.length - 1, values.length - headers.length + 1, values.slice(headers.length - 1).join(','));
    return Object.fromEntries(headers.map((header, index) => [header, values[index] ?? '']));
  });
}

function resourceKey(name: string) { return `resource:${name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`; }
function projectRow(project: ProjectEntity): ConstructionTask {
...
export function buildConstructionModel(input = { tasksCsv, resourcesCsv, lookAheadCsv, dependenciesCsv }): ConstructionModel {
  ...
  const canonical = sourceTasks.filter(({ type }) => type !== 'project').map((row): ConstructionTask => {
    ...
    return { id: taskId(row.project_ref, row.id), parentId, projectRef: row.project_ref, legacyTaskId: row.id, entityKind: 'task', source: 'master', name: row.name, type: row.type === 'phase' ? 'summary' : row.type, startDate: row.start, endDate: row.finish, percentDone: asNumber(row.percent_complete), workflowStatus: status('', asNumber(row.percent_complete)), statusLabel: status('', asNumber(row.percent_complete)), calendarId: `cal-${resourceByName.get(row.owner)?.calendarId?.replace(/^cal-/, '') || 'office_mon_fri'}`, department, departmentLabel: departmentLabel[department], resourceName: row.owner, notes: row.notes, tags: [row.wbs, department] };
  });
...
export function trimmedLookAheadRows(source: ConstructionTask[], period: LookAheadPeriod, filters: LookAheadFilters): Record<number, boolean> {
  const included = new Set<string>(); const byId = new Map(source.map((task) => [task.id, task]));
  for (const task of source) { const overlaps = String(task.startDate) <= period.end && String(task.endDate) >= period.start; const department = filters.department === 'all' || task.department === filters.department; const area = filters.workArea === 'all' || task.workArea === filters.workArea; if (overlaps && department && area && task.type !== 'summary') { let current: ConstructionTask | undefined = task; while (current) { included.add(current.id); current = current.parentId ? byId.get(String(current.parentId)) : undefined; } } }
  return Object.fromEntries(source.map((task, index) => [index, !included.has(task.id)]));
}
...
export function applyConstructionTaskPatch(tasks: ConstructionTask[], detail: any): ConstructionTask[] { if (!detail?.sourcePatch || detail.taskId == null) return tasks; return tasks.map((task) => task.id === String(detail.taskId) ? { ...task, ...detail.sourcePatch } : task); }
